...
(define-public (create-project (project-name (string-ascii 100)) (project-description (string-ascii 256)) (project-goal uint))
  (begin
  ...
  (begin
    (let ((project (map-get? projects project-id)))
      ...
      (let ((current-raised (get raised (unwrap-panic project))))
        ...
        (map-set projects project-id (merge (unwrap-panic project) { raised: (+ current-raised amount) }))
